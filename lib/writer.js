      ...
      Object.keys(this.types[type]).forEach(function (category) {
        ...
        types[type][category].forEach(function (commit) {
          ...
          if (
            ...
            commit.body.startsWith("https://app.asana.com")
          ) {
            asanaLink = ` ,[asana task 🔗](${commit.body})`;
          }
          // if commit.body start with https://app.asana.com then add a new line with the body as markdown link

          content.push(
            prefix + " " + subject + " (" + shorthash + ")" + asanaLink
          );
        });
      });
